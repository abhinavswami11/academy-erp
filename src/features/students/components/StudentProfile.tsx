...
export default function StudentProfile({ student }: StudentProfileProps) {
  ...
  const feePayments = useMemo(
    () =>
      feeRecords.flatMap((fee) => getPaymentsByFeeId(fee.id)),
    [feeRecords],
  );

  const todayAttendance = useMemo(() => {
    const records = buildAttendanceForDate(getToday());
    return records.find((record) => record.studentId === student.id);
  }, [student.id]);

  const savedAttendanceCount = useMemo(() => {
    return getAttendance(getToday()).filter(
